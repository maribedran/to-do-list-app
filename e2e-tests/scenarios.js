...
describe('ToDoList Application', function() {
  ...
  describe('View: ToDoList detail', function() {
    ...
    beforeEach(function() {
      browser.get('index.html#!/to-do-lists/nexus-s');
    });
    ...
    it('should display the first toDoList image as the main toDoList image', function() {
      var mainImage = element(by.css('img.toDoList.selected'));

      expect(mainImage.getAttribute('src')).toMatch(/img\/to-do-lists\/nexus-s.0.jpg/);
    });

    it('should swap the main image when clicking on a thumbnail image', function() {
      var mainImage = element(by.css('img.toDoList.selected'));
      var thumbnails = element.all(by.css('.toDoList-thumbs img'));

      thumbnails.get(2).click();
      expect(mainImage.getAttribute('src')).toMatch(/img\/to-do-lists\/nexus-s.2.jpg/);
